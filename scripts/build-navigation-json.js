        ...
        const documentationData = documentationCss.extract(cssFilePaths.map((filePath) => {
          ...
          if (obj.parsedComment && obj.parsedComment.source) {
            ...
            if (obj.parsedComment.source.filename) {
              obj.parsedComment.source.filename = path.basename(obj.parsedComment.source.filename);
            }
          }
...
function buildNavigationJSON(options) {
  ...
  return pify(mkdirp)(outdir)
    .then(() => {
      return pify(fs.writeFile)(path.join(outdir, outfile), JSON.stringify({ navigationList }), 'utf8');
    })
