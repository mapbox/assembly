(function () {
  ...
  Assembly.iconExists = function (iconName) {
    return new RegExp('id=[\'"]icon-' + iconName + '[\'"]').test(Assembly._svgSprite);
  };

  function assertIcon(iconName) {
    if (!Assembly.iconExists(iconName)) throw new Error('Icon "' + iconName + '" does not exist');
  }
  ...
  Assembly.changeIcon = function (iconEl, iconName) {
    assertIcon(iconName);
    iconEl.firstChild.setAttributeNS(xlinkNS, 'xlink:href', '#icon-' + iconName);
    return iconEl;
  };
}());
